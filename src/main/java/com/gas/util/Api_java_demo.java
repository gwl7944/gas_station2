...
public class Api_java_demo {
    ...
    public static final String SN = "520544226";//*必填*：打印机编号，必须要在管理后台里添加打印机或调用API接口添加之后，才能调用API
    ...
    public static void main(String[] args) throws Exception {
        ...
        printer.setPrinter_code(SN);
        printer.setPrinter_user(USER);
        printer.setPrinter_Identification_code(UKEY);
        //String method1 = print2(printer,records_consumption);
        //String method1 = print3(printer,stationPrintInfo);
        String snlist = "52054422dhahdh#rc6gmz9i#侯马加油站";  //打印机编号  打印机识别码  备注
        //String method1 = addprinter2(printer,snlist);
        ...
        //tring method3 = queryOrderState(orderid);
     ...
     * 添加打印机
     */
    public static String addPrinter2(Printer printer, String snlist) {
        //***返回值JSON字符串***
        //正确例子：{"msg":"ok","ret":0,"data":{"ok":["sn#key#remark#carnum","316500011#abcdefgh#快餐前台"],"no":["316500012#abcdefgh#快餐前台#13688889999  （错误：识别码不正确）"]},"serverExecutedTime":3}
        //错误：{"msg":"参数错误 : 该帐号未注册.","ret":-2,"data":null,"serverExecutedTime":37}

        //提示：打印机编号(必填) # 打印机识别码(必填) # 备注名称(选填) # 流量卡号码(选填)，多台打印机请换行（\n）添加新打印机信息，每次最多100行(台)。
        //String snlist = "sn1#key1#remark1";  //打印机编号  打印机识别码  备注
        String method = addprinter2(printer, snlist);
        System.out.println(method);
        return method;
    }
     ...
     * 查询打印机状态
     */
    public static String queryPrinterStatus2(String code) {
        //***返回值JSON字符串***
        //成功：{"msg":"ok","ret":0,"data":"状态","serverExecutedTime":4}
        //失败：{"msg":"错误描述","ret":非0,"data":"null","serverExecutedTime":5}
        String method5 = queryPrinterStatus(code);
        System.out.println(method5);
        return method5;
    }
    //=====================以下是函数实现部分================================================
    ...
    public static String addprinter2(Printer printer, String snlist) {
        ...
        HttpPost post = new HttpPost(URL);
        List<NameValuePair> nvps = new ArrayList<NameValuePair>();
        nvps.add(new BasicNameValuePair("user", printer.getPrinter_user()));
        String STIME = String.valueOf(System.currentTimeMillis() / 1000);
        nvps.add(new BasicNameValuePair("stime", STIME));
        nvps.add(new BasicNameValuePair("sig", signature(printer.getPrinter_user(), printer.getPrinter_Identification_code(), STIME)));
        nvps.add(new BasicNameValuePair("apiname", "Open_printerAddlist"));//固定值,不需要修改
        nvps.add(new BasicNameValuePair("printerContent", snlist));
    ...
    //消费打印
    public static String print2(Printer printer, Records_consumption recordsConsumption) {
        System.out.println(printer + "," + recordsConsumption);
        //标签说明：
        //单标签:
        //"<BR>"为换行,"<CUT>"为切刀指令(主动切纸,仅限切刀打印机使用才有效果)
        //"<LOGO>"为打印LOGO指令(前提是预先在机器内置LOGO图片),"<PLUGIN>"为钱箱或者外置音响指令
        ...
        //根据打印纸张的宽度，自行调整内容的格式，可参考下面的样例格式
        //标题
        String rc_sitecode_name = recordsConsumption.getRc_sitecode_name();
        //交易时间
        String rc_date_str = recordsConsumption.getRc_Date_str();
        //订单号
        String rc_number = recordsConsumption.getRc_number();
        //是否首次打印
        //Integer rc_print = recordsConsumption.getRc_print();
        String yes_no = "是";
        //油枪
        String rc_oil_gun = recordsConsumption.getRc_oil_gun();
        //油品
        ...
        if (recordsConsumption.getRc_wu_id() == null || recordsConsumption.getRc_consumer_projects_code() == null) {
            mode_of_payment = "微信支付";
        } else {
            mode_of_payment = "余额支付";
        }
        ...
        //content += "名称　　　　　 单价  数量 金额<BR>";
        //content += "--------------------------------<BR>";
        content += "交易时间:" + rc_date_str + "<BR>";
        content += "订单号:" + rc_number + "<BR>";
        content += "是否首次打印:" + yes_no + "<BR>";
        ...
        content += "付款方式:" + mode_of_payment + "<BR>";
        if (phone != null) {
        ...
        nvps.add(new BasicNameValuePair("sig", signature(printer.getPrinter_user(), printer.getPrinter_Identification_code(), STIME)));
        nvps.add(new BasicNameValuePair("apiname", "Open_printMsg"));//固定值,不需要修改
        nvps.add(new BasicNameValuePair("sn", printer.getPrinter_code()));
        nvps.add(new BasicNameValuePair("content", content));
        nvps.add(new BasicNameValuePair("times", "1"));//打印联数
    ...
    //结班打印
    public static String print3(Printer printer, StationPrintInfo stationPrintInfo) {
        //标签说明：
        //单标签:
        //"<BR>"为换行,"<CUT>"为切刀指令(主动切纸,仅限切刀打印机使用才有效果)
        //"<LOGO>"为打印LOGO指令(前提是预先在机器内置LOGO图片),"<PLUGIN>"为钱箱或者外置音响指令
        ...
        //根据打印纸张的宽度，自行调整内容的格式，可参考下面的样例格式
        //标题
        String rc_sitecode_name = stationPrintInfo.getSite_name();
        //营业日
        String business_day = stationPrintInfo.getBusiness_day();
        //开班
        String user_onduty_time_str = stationPrintInfo.getUser_onduty_time_str();
        //结班
        String user_offduty_time_str = stationPrintInfo.getUser_offduty_time_str();
        //成交数
        Integer fixture_number = stationPrintInfo.getFixture_number();
        //是否首次打印
        Integer print_num = stationPrintInfo.getPrint_num();
        String yes_no = "否";
        if (print_num != null) {
        ...
        //操作人
        String operator = stationPrintInfo.getOperator();

        /*----------- 充值合计 -----------*/
        //应收总额
        Double top_up_amounp = stationPrintInfo.getTop_up_amounp();
        //实收总额
        Double paid_amount = stationPrintInfo.getPaid_amount();
        //优惠金额
        Double coupon_amount = stationPrintInfo.getCoupon_amount();
        /*----------- 消费合计 -----------*/
        //应收总额
        Double sales_amount = stationPrintInfo.getSales_amount();
        //实收总额
        Double sales_official_amount = stationPrintInfo.getSales_official_amount();
        //优惠总额
        Double sales_coupon_amount = stationPrintInfo.getSales_coupon_amount();
        //无卡消费总额
        Double sales_nocard_amount = stationPrintInfo.getSales_nocard_amount();
        //总销量
        Double sales_volume = stationPrintInfo.getSales_volume();


        String content;
        content = "<CB>" + rc_sitecode_name + "</CB><BR>";
        content += "营业日:" + business_day + "<BR>";
        content += "开班:" + user_onduty_time_str + "<BR>";
        content += "结班:" + user_offduty_time_str + "<BR>";
        content += "成交数:" + fixture_number + "<BR>";
        content += "是否首次打印:" + yes_no + "<BR>";
        content += "操作人:" + operator + "<BR>";
        content += "--------------------------------<BR>";
        content += "<B>" + "充值合计" + "</B><BR>";
        content += "应收总额:" + top_up_amounp + "<BR>";
        content += "实收总额:" + paid_amount + "<BR>";
        content += "优惠总额:" + coupon_amount + "<BR>";
        content += "--------------------------------<BR>";
        content += "<B>" + "消费合计" + "</B><BR>";
        content += "应收总额:" + sales_amount + "<BR>";
        content += "实收总额:" + sales_official_amount + "<BR>";
        content += "优惠总额:" + sales_coupon_amount + "<BR>";
        content += "无卡消费总额:" + sales_nocard_amount + "<BR>";
        content += "总销量(L):" + sales_volume + "<BR>";
        content += "--------------------------------<BR>";
        List<OilPrint> oilPrintList = stationPrintInfo.getOilPrintList();
        for (OilPrint oilPrint : oilPrintList) {
            content += "<B>" + oilPrint.getOil_name() + ":" + oilPrint.getOil_unit_price() + "元/L" + "</B><BR>";
            content += "应收总额:" + oilPrint.getReceivable_amount() + "<BR>";
            content += "实收总额:" + oilPrint.getPaid_amount() + "<BR>";
            content += "优惠总额:" + oilPrint.getCoupon_amount() + "<BR>";
            content += "销售量(L):" + oilPrint.getSales_volume() + "<BR>";
        }
        content += "--------------------------------<BR>";
        content += "<C>" + "软件服务由嘉联支付山西分公司提供" + "</C>";
        //content += "送货地点：广州市南沙区xx路xx号<BR>";
